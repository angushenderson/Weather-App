...
class Forecast:
    ...
    def detailed_hour_rain_forecast(self, val):
        pass
    ...
    def detailed_3_hour_5_day_forecast(self):
        ...
        self._detailed_3_hour_5_day_forecast = self._get_json(
            "_detailed_3_hour_5_day_forecast",
            f"https://api.openweathermap.org/data/2.5/forecast?lat={self.location.coords[0]}&lon={self.location.coords[1]}&appid={self.API_KEY}&units={self.units}",
        )
...
class Location:
    ...
    def search_suggestions(self, search_text: str) -> list[dict]:
        ...
        if len(search_text) > 0:
            locations = list(
                filter(
                    lambda item: search_text in item["name"].lower(),
                    location_data[search_text[0].lower()],
                )
            )
            return (
                locations
                if len(locations) <= self.SEARCH_LIMIT
                else locations[: self.SEARCH_LIMIT]
            )
        else:
